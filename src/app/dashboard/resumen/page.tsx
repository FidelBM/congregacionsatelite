...
const TableThree = () => {
    ...
    const tableData = filteredUsers.map(user => ({
        ...
        precursorado: cards.filter(card => card.userId === user.id && card.auxiliar).length > 0 ? 'Precursor Auxiliar' : user.precursorado,
        predico: cards.filter(card => card.userId === user.id).map(card => card.predico ? 'Si' : 'No').join(', '),
        cursos: cards.filter(card => card.userId === user.id).map(card => card.cursos?.toString()).join(', '),
        horas: cards.filter(card => card.userId === user.id).map(card => card.horas?.toString()).join(', '),
        comentarios: cards.filter(card => card.userId === user.id).map(card => card.comentarios?.toString()).join(', ')
    }));
    ...
    return (
        ...
        <DefaultLayout>
            ...
            <div className="flex flex-col gap-10">
                <button onClick={exportToExcel} className='bg-green-300 p-5 rounded text-black-2 font-bold'>
                    Descargar Excel
                </button>
                ...
                <div
                    ...
                    <div className="max-w-full overflow-x-auto">
                        ...
                        <table className="w-full table-auto">
                            ...
                            <tr>
                                ...
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p className="text-black dark:text-white">
                                        {users.filter(user => user.precursorado === 'Publicador').length - cards.filter(card => card.auxiliar).length}
                                    </p>
                                </td>
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <h5 className="text-black dark:text-white">
                                        {cards.filter(card => card.user.precursorado === 'Publicador' && !card.auxiliar).length}

                                    </h5>
                                </td>
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p className="text-black dark:text-white">
                                        {cards.filter(card => card.user.precursorado === 'Publicador' && !card.auxiliar).map(card => card.cursos).reduce((acc, curr) => acc + (curr ? parseInt(curr) : 0), 0)}
                                    </p>
                                </td>
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p className="text-black dark:text-white">
                            ...
                            <tr>
                                ...
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p className="text-black dark:text-white">
                                        {users.filter(precursores => precursores.precursorado === 'Precursor Regular').length}
                                    </p>
                                </td>
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <h5 className="text-black dark:text-white">
                                        {cards.filter(card => card.user.precursorado === 'Precursor Regular').length}

                                    </h5>
                                </td>
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p className="text-black dark:text-white">
                                        {cards.filter(card => card.user.precursorado === 'Precursor Regular').map(card => card.cursos).reduce((acc, curr) => acc + (curr ? parseInt(curr) : 0), 0)}
                                    </p>
                                </td>
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p className="text-black dark:text-white">
                            ...
                            <tr>
                                ...
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p className="text-black dark:text-white">
                                        {cards.filter(card => card.auxiliar).map(card => card.cursos).reduce((acc, curr) => acc + (curr ? parseInt(curr) : 0), 0)}
                                    </p>
                                </td>
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p className="text-black dark:text-white">
                            ...
                            <tr>
                                ...
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p className="font-medium text-black dark:text-white">
                                        {cards.map(card => card.cursos).reduce((acc, curr) => acc + (curr ? parseInt(curr) : 0), 0)}
                                    </p>
                                </td>
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p className="font-medium text-black dark:text-white">
                    ...
                    <div className="max-w-full overflow-x-auto">
                        ...
                        <table className="w-full table-auto" id="promedio">
                            ...
                            <tr>
                                ...
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p className="font-medium text-black dark:text-white">
                                        {((cards.map(card => card.cursos).reduce((acc, curr) => acc + (curr ? parseInt(curr) : 0), 0)) / (cards.length)).toFixed(2)}
                                    </p>
                                </td>
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p className="font-medium text-black dark:text-white">
                    ...
                    <div className="max-w-full overflow-x-auto mt-4">
                        <h1 className='text-2xl text-black-2 font-bold m-5'>Publicadores</h1>
                        <table className="w-full table-auto">
                            <thead>
                            <tr className="bg-gray-2 text-left dark:bg-meta-4">
                                <th className="px-4 py-4 font-medium text-black dark:text-white">
                            ...
                            {filteredUsers.map((user, key) => (
                                <tr key={key}>
                                    ...
                                    <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                        <p className="text-black dark:text-white">
                                            {cards.filter(card => card.userId === user.id).map(card => card.predico ? 'Si' : 'No')}
                                        </p>
                                    </td>
                                    <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                        <p className="text-black dark:text-white">
                                            {cards.filter(card => card.userId === user.id).map(card => card.cursos?.toString())}
                                        </p>
                                    </td>
                                    <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                        <p className="text-black dark:text-white">
                                            {cards.filter(card => card.userId === user.id).map(card => card.horas?.toString())}
                                        </p>
                                    </td>
                                    <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                        <p className="text-black dark:text-white">
                                            {cards.filter(card => card.userId === user.id).map(card => card.comentarios?.toString())}
                                        </p>
                                    </td>

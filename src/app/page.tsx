...
const FormLayout = () => {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        if (user) {
            ...
            const response2 = await fetch(`${process.env.API_URL}/cards`);
...
// 3. Filtra las tarjetas del usuario específico por el mes actual
            ...
            if (currentMonthCards.length >= 1) {
                ...
                const response = await fetch(`${process.env.API_URL}/cards/${currentMonthCards[0].id}`, {
                    method: 'PUT',
                    headers: {
                ...
                console.log(`http://localhost:8000/cards/${currentMonthCards[0].id}`)
                if (!response.ok) {
    ...
    return (
        ...
        <div className="grid grid-cols-1 gap-9 sm:grid-cols-1">
            <div className="flex flex-col gap-9 md:justify-center md:items-center md:pt-11">
                {/* <!-- Contact Form --> */}
                <h1 className="font-bold text-2xl text-center pt-5 md:pt-0">Congregacion Cd. Satélite - Tlalnepantla</h1>
